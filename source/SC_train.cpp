...
int argmax(const vector <float>& m) {
	...
	return distance(m.begin(), std::max_element(m.begin(), m.end()));
}
...
vector <float> operator/(const vector <float>& m2, const vector <float>& m1) {
...
int main(int argc, const char* argv[]) {
	...
	int conf_matrix[10][10]={0};

	// Random initialization of the weights
	int W1_size=128; //number of neurons on HL1
	int W2_size=64; //number of neurons on HL2
	int indez=0;
	vector <float> W1 = random_vector(784 * W1_size);
	vector <float> W2 = random_vector(W1_size * W2_size);
	vector <float> W3 = random_vector(W2_size * 10);
	cout << "Training the model ...\n";
	for (unsigned i = 0; i < 100000; ++i) {
		srand(time(NULL));
		it++;
		// Building batches of input variables (X) and labels (y)
		if(indez+BATCH_SIZE<28000){
			indez+=BATCH_SIZE;
		}
		else indez=0;
		vector<float> b_X;
		vector<float> b_y;
		for (unsigned j = indez * 784; j < (indez + BATCH_SIZE) * 784; ++j) {
			b_X.push_back(X_train[j]);
		}
		for (unsigned k = indez * 10; k < (indez + BATCH_SIZE) * 10; ++k) {
			b_y.push_back(y_train[k]);
		}

		// Feed forward
		vector<float> a1 = sigmoid(dot(b_X, W1, BATCH_SIZE, 784, W1_size));
		for(int j=0;j<BATCH_SIZE;j++){
			for(int i=0;i<int(W1_size*0.15);i++){
            	int randind=rand()%W1_size;
				a1[randind+j*W1_size]=0.000000000000001;
        	}
		}
		vector<float> a2 = sigmoid(dot(a1, W2, BATCH_SIZE, W1_size, W2_size));
		for(int j=0;j<BATCH_SIZE;j++){
			for(int i=0;i<int(W2_size*0.15);i++){
            	int randind=rand()%W2_size;
				a2[randind+j*W2_size]=0.000000000001;
        	}
		}
		...
		for (int j = 0; j < BATCH_SIZE; j++) {
			...
			if (b_y[max_ind+j*10] == 1) {
				hit++;
				conf_matrix[max_ind][max_ind]++;
			}
			else{
				conf_matrix[max_ind][true_ind]++;
			}
		}
		// Back propagation

		vector<float> dyhat = 2*(yhat-b_y);

		// dW3 = a2.T * dyhat
		vector<float> dW3 = dot(transpose(&a2[0], BATCH_SIZE, W2_size), dyhat, W2_size, BATCH_SIZE, 10);
		// dz2 = dyhat * W3.T * relu'(a2)
		vector<float> dz2 = dot(dyhat, transpose(&W3[0], W2_size, 10), BATCH_SIZE, 10, W2_size) * sigmoid_d(a2);
		// dW2 = a1.T * dz2
		vector<float> dW2 = dot(transpose(&a1[0], BATCH_SIZE, W1_size), dz2, W1_size, BATCH_SIZE, W2_size);
		// dz1 = dz2 * W2.T * relu'(a1)
		vector<float> dz1 = dot(dz2, transpose(&W2[0], W1_size, W2_size), BATCH_SIZE, W2_size, W1_size) * sigmoid_d(a1);
		// dW1 = X.T * dz1
		vector<float> dW1 = dot(transpose(&b_X[0], BATCH_SIZE, 784), dz1, 784, BATCH_SIZE, W1_size);
		...
		W1 = W1 - lr * dW1 ;
		if ((i + 1) % 100 == 0) {
			accuracy =(float) hit / (it*BATCH_SIZE);
			cout << "-----------------------------------------------Epoch " << i + 1 << "--------------------------------------------------" << "\n";
			cout << "Predictions:" << "\n";
			print(yhat, 10, 10);
			cout << "Ground truth:" << "\n";
			...
			for(int i=0;i<10;i++){
				...
				if(total==0) res=0;
				else res=float(conf_matrix[i][i])/total*100;
	...
	//Printing weights
	ofstream outweights;
	outweights.open("../data/Weights.txt");
	if (!outweights) {
		cout << "pain\n";
	}
	cout << "made file\n";
